...
magnisium <- function(volumn, finalconc,times_buffer =10, Mg_in_buffer = 15,wsolution = 25) {
    ...
    x = list()
    for (i in volumn) {
        x[[i]] = (finalconc*i- i/times_buffer*Mg_in_buffer)/wsolution
        names(x[[i]]) = finalconc
    }
    return(x)    
}
...
# times_buffer: the Reaction Buffer solution, for example 10 × Pol Buffer B for OptiTag from roboklon (see http://www.roboklon.de/pdf/7_en.pdf for this Buffer);
# Mg_in_buffer: the Mg concentration already in buffer, e.g. 10 x Pol Buffer B (general 
application, up to 8-10 kb) contains 15 mM MgCl2
...
write.table(cc, file='MgCl2_to_be_added.txt',quote=F,sep="\t")
